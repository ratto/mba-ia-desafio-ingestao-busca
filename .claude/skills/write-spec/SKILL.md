...
Always generate two files inside a single folder called "[feature number]-[feature resume in 4 words or less]" (like "F01-implement-search-engine") inside /docs folder. The two output files must be:

- **SPEC.md**: (the why) markdown file containing a high level design of the feature covering business domain aspects.
- **PLAN.md**: (the how) markdown file with a detailed plan for the feature implementation.
